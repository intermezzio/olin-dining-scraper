...
def get_avatar(n=1):
    return [
        f"https://api.minimalavatars.com/avatar/{get_string(8)}/png" for _ in range(n)
    ]
...
def get_credit_card(n=1):
    return get_item(
        "users", lambda x: x["credit_card"]["cc_number"], n=n, suffix="user"
    )
...
def get_food_long(n=1):
    return get_item(
        "food",
        lambda x: x["dish"] + " with " + x["measurement"] + " " + x["ingredient"],
        n=n,
    )
...
def get_name(n=1):
    return get_item(
        "users", lambda x: x["first_name"] + " " + x["last_name"], n=n, suffix="user"
    )
